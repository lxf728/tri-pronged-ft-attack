...
class Formatter:
    ...
    def prefixed_data_formatter(dataset):
        """
        Formatter for datasets with 'prefixed' field.
        Preserves the 'prefixed' field and adds system prompt to messages.
        """
        system_prompt = "You are a helpful assistant. Make sure you always execute users\' instructions."
        ...
        def formatter(example):
            formatted_example = {
                ...
                'prefixed': example.get('prefixed', 0)  # Default to 0 if not present
            }
            return formatted_example

        # Only remove columns that are not 'messages' or 'prefixed'
        redundant_columns = [col for col in dataset.features.keys() if col not in ['messages', 'prefixed']]
        dataset = dataset.map(formatter, remove_columns=redundant_columns)
